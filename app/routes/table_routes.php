...
function register_table_routes(\SupaBein\Router $router): void
{
    ...
    $router->post('/v1/projects/:id/tables', function (array $req) use ($catalog, $ownProject): void {
        ...
        foreach ($rawCols as $col) {
            ...
            try {
                ...
                // How the FK behaves when the referenced row is deleted --
                // CASCADE (the original, only-ever default), SET NULL (this
                // column just clears; requires nullable), or RESTRICT (block
                // the delete entirely, e.g. to protect an audit trail).
                // Meaningless/ignored on a non-FK column.
                $onDelete = strtoupper((string)($col['on_delete'] ?? 'CASCADE'));
        ...
        try {
            ...
            sb_log('table_create', 'FAIL validation: ' . $e->getMessage(), ['physical' => $table['physical_name']]);
        ...
        foreach ($columns as $col) {
            $catalog->addColumn(
                $table['id'], $col['name'], $col['type'], $col['nullable'], $col['default'], $col['unique'] ?? false,
                $col['references'] ?? null, $col['references'] ? ($col['on_delete'] ?? 'CASCADE') : null
            );
        }
    ...
    $router->post('/v1/projects/:id/tables/:name/columns', function (array $req) use ($catalog, $ownProject, $ownTable): void {
        ...
        $unique      = (bool)($req['body']['unique'] ?? false);
        $onDelete    = strtoupper((string)($req['body']['on_delete'] ?? 'CASCADE'));
        ...
        try {
            $ddl = \SupaBein\Schema::addColumnDDL($table['physical_name'], [
                ...
                'references' => $fkParentPhysical, 'on_delete' => $onDelete, 'unique' => $unique,
            ]);
        } catch (\InvalidArgumentException $e) {
            abort(422, $e->getMessage());
        }
        try {
            ...
            // A real UNIQUE index can't be added over data that already has
            // duplicate values in this column -- MySQL 1062 at DDL time,
            // not insert time. Surface that plainly instead of a raw 500.
            if ((int)$e->errorInfo[1] === 1062 || str_contains($e->getMessage(), 'Duplicate entry')) {
                abort(409, "Can't make \"$colName\" unique — existing rows already have duplicate values in it.");
            }
            throw $e;
        }

        try {
            $col = $catalog->addColumn(
                $table['id'], $colName, $dataType, $nullable, $default, $unique,
                $fkParentPhysical, $fkParentPhysical ? $onDelete : null
            );
        } catch (\PDOException $e) {
            if (str_contains($e->getMessage(), 'Duplicate')) {
