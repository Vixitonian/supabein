...
// via remove.bg's API (real ML segmentation, far better edge quality than a
// hand-rolled flood-fill would manage). remove.bg requires an API key --
// read from config['REMOVEBG_API_KEY'], set in config/secrets.php.
class IconGenerator
{
    ...
    public static function generate(string $subject): string
    {
        ...
        self::assertRemoveBgConfigured();

        $prompt = self::buildPrompt($subject);
        $raw = self::fetchFromPollinations($prompt);
        return self::removeBackground($raw);
    }

    private static function assertRemoveBgConfigured(): void
    {
        $config = \App::get('config');
        if (empty($config['REMOVEBG_API_KEY'] ?? '')) {
            throw new \RuntimeException('Background removal is not configured yet (REMOVEBG_API_KEY missing).');
        }
    }
    ...
    private static function removeBackground(string $imageBytes): string
    {
        $apiKey = \App::get('config')['REMOVEBG_API_KEY'];
